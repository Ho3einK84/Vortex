...
- **Lazy-loaded QR generator.** `qrcode-generator` (~21KB minified — a quarter of the whole app bundle) is now built as its own small ES module instead of being bundled into `app.js`. It's embedded as a second, inert base64 blob and only decoded + dynamically imported from a `Blob` URL (no network request, same self-contained guarantee) the first time a user actually opens a QR modal. Total file size is unchanged, but initial script parse/execution work is meaningfully lighter for the common case where a user never opens a QR code. The modal opens instantly either way, showing a brief "Generating…" placeholder on the very first open (subsequent opens are cached and instant); a load failure falls back to a "use Copy instead" message rather than a broken modal.
- Verified with a rebuilt `dist/index.html` (400KB → 330.7KB), the existing visual-regression checks, manual light/dark + EN/FA screenshots, and a real-browser (Playwright/Chromium) check that the lazy QR import renders a valid SVG with no console errors on both first and cached opens.
